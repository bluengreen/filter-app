<script>
  ...
  let selectedTrend = watcher(null, () => page = 1)
  let query
  let isLoading = true
  let viewStyle = "tiles"
...
<div class="news-items--wrapper news-items--{viewStyle}">
  ...
  {/each}
  {#if loadEndlessly }
  <SvelteInfiniteScroll threshold={100} on:loadMore={() => console.log("MORE") || page++} {hasMore} window={true}/>
  {:else if hasMore}
    <div class='text-center'>
      <button class='btn btn-primary' on:click={e => loadEndlessly = true }>Nachladen aktivieren</button>
    </div>
  {/if}
</div>
